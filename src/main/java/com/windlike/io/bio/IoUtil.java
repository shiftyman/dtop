...
//    }
    ...
    public static void readTopNUser(DataInputStream input, int[] userNumList, int expertSize) throws IOException {
        int bufsize = 1024;
        byte[] bytes = new byte[bufsize];
        int length;
        int off = 0;
        ...
        do{
            ...
            if(off == bufsize || leftBytes == 0){
                //读取
                for(int i = 0; i < off; i+=4){
                    userNumList[readTime] = (((bytes[i] & 0x000000FF) << 24)) + ((bytes[i+1] & 0x000000FF ) << 16) + ((bytes[i+2]& 0x000000FF) << 8) + ((bytes[i+3]& 0x000000FF) << 0);
